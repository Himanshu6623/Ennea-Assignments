...
export default function Courses() {
    ...
    const [totalCoursesSubscribed,setTotalCoursesSubscribed]=useState(0)
    ...
    const { refetch: refetchSubscribedCourse } = useQuery('fetchSubscribedData', () =>
      axios.get(`http://localhost:8080/Student/SubscribedCourses/${userDetails.rollno}`,{
        ...
        if (data) {
          ...
          setTotalCoursesSubscribed(data.length)
        }
      }}
    );
    ...
    const { error, isLoading, refetch: refetchCourse} = useQuery('fetchData', () =>
     ...
     const { mutate,Loading } = useMutation(Subscribe, {
       onSuccess: (data) => {
    ...
    const changePage=(event,Id)=>{
        ...
        {
            if(totalCoursesSubscribed===2)
            {
              notification.warning({
                message: 'Maximum Subscription limit reached',
                description:"Each person can once subscribe a max of 2 courses ",
                placement: 'topRight',
                duration: 3,
              });
              return
